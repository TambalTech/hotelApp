...
const ExclusiveOffers = () => {
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-20 pb-30'>
        <div className='flex flex-call md:flex-row items-center justify-between w-full'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
            {exclusiveOffers.map((item)=>(
                ...
                style={{backgroundImage:`url(${item.image})`}}>
                    <p className='px-3 py-1 absolute top-4 text-xs left-4 bg-white text-gray-800 font-medium rounded-full'>{item.priceOff}% OFF</p>
                    <div>
                        <p className='text-xl font-playfair font-medium'>{item.title}</p>
